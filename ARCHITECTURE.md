...
* **Strict Priority Hierarchy & Resilience Architecture:**
  1. *Tier 1 — Online Primary (Sarvam AI Bulbul:v3):* Whenever the client has internet connectivity (`navigator.onLine === true`), whether routed via backend or edge swarm, speech synthesis is executed through Sarvam AI (`POST https://api.sarvam.ai/text-to-speech`) with automated retries and character length clamping (<490 chars) matching `tts_client.py`. If client-side direct fetch is blocked by carrier firewalls, a secondary backend proxy route (`POST /api/v1/tts`) seamlessly fulfills synthesis.
  2. *Tier 2 — Automated Quota Failover to Native Speech (`window.speechSynthesis`):* If Sarvam AI returns HTTP 429 (`Too Many Requests`), 402 (`Payment Required`), 403 (`Forbidden`), or daily quota limits, the client flags `isSarvamQuotaExhausted = true` for the session and immediately fast-paths subsequent calls directly to the device's native Web Speech API (`speakVernacularOffline(text, lang, true)`) without futile delays. Replay buttons, file selection callbacks, and `<audio onError={...}>` listeners all fail over seamlessly so spoken advisories are never muted.
  3. *Tier 3 — Mobile Autoplay Compliance & Gesture Activation:* To comply with iOS/Android media autoplay policies, the Farmer UI incorporates interactive replay controls, enabling instantaneous acoustic playback within a valid user touch context.
  4. *Tier 4 — Offline Autonomous Fallback (`window.speechSynthesis`):* In true zero-connectivity agrarian dead zones (`!navigator.onLine`), speech is generated locally on the browser thread with zero network dependencies.
...
- **Gemini Multi-Model Fallback & Uncertified Crop Direct Routing:** When CV inference is uncertain (< 97% confidence or < 60% margin) or crop is uncertified (e.g. Guava, Hibiscus), the swarm cascades across `gemini-flash-lite-latest`, `gemini-flash-latest`, and `gemini-3.6-flash` with zero-wait retry failover (`max_retries=0`, `timeout=15.0`) right inside Agent 1. This prevents LangChain 429 retry sleeps, dropping round-trip execution from 156s down to 8.8s and eliminating client fetch aborts. If uncertified, LangGraph conditional edges immediately route to Agent 5 (Voice/Advisory), bypassing RAG/Safety to lock chemical advice and issue official KVK referrals with practical organic care guidance.
...
- **Sarvam AI Quota Failover to Mobile Internal Speech:** Automated zero-latency fallback from Sarvam AI cloud synthesis to on-device mobile internal speech (`window.speechSynthesis`) across all 11 Indian languages whenever HTTP 429, 402, 403, or quota limits are encountered. Caches quota exhaustion state per session to bypass retry delays, un-suppresses device speech when online during failover (`allowOnlineFallback: true`), and guards `<audio>` playback with inline error recovery.
